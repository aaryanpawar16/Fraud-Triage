...
micro1 Agentic Workflows Hackathon. No prior codebase was carried in.

[![CI](https://github.com/aaryanpawar16/Fraud-Triage/actions/workflows/ci.yml/badge.svg)](https://github.com/aaryanpawar16/Fraud-Triage/actions/workflows/ci.yml)
...
- **All 45 tests, every one of them** — not a subset. This is possible
  because every test file is genuinely independent of a live model:
  tool logic and pattern detection are pure deterministic functions,
  and everywhere an LLM call would otherwise be needed, `MockClient`
  stands in with a scripted response. **No API key or secret is
  configured in this repo's CI at all** — the badge above is real
  green, not green because a step was skipped.
- **The website's full production build**, including a standalone
  `tsc --noEmit` type-check pass before the build even starts, so a
  type error fails fast instead of surfacing 90 seconds into a build.

Both jobs run on a clean `ubuntu-latest` runner — a third, independent
verification environment beyond the two machines mentioned above.
...
trajectories/     per-case traces (run_eval.py) + Tier 3 and adversarial trajectories
tests/            45 tests across 8 files
docs/             CHANGELOG.md, REPRODUCTION.md, SCORECARD.md, ARCHITECTURE.md
website/          Next.js site, 11 pages, live demo included
.github/          CI workflow — runs all 45 tests + website build on every push
