...
import EvidenceTrailReveal from "@/components/EvidenceTrailReveal";
...
export function generateMetadata({ params }: { params: { id: string } }): Metadata {
  ...
  if (!c) return { title: "Case not found — Fraud Triage Agent" };
  return {
    title: `${c.case_id} — Fraud Triage Agent`,
    description: `${c.transaction.merchant}, $${c.transaction.amount_usd} — agent verdict: ${c.agent.verdict}.`,
  };
}
...
export default function CaseDetailPage({ params }: { params: { id: string } }) {
  ...
  return (
    <main className="mx-auto max-w-4xl px-6 py-16">
      <Link href="/case-list" className="font-mono text-xs text-text-dim hover:text-text">
        &larr; all cases
      </Link>
      ...
      <section className="mt-6 rounded-md border border-border bg-surface p-6">
        ...
        <EvidenceTrailReveal evidenceLog={c.agent.evidence_log} />
      </section>
