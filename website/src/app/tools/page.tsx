...
export default function ToolsPage() {
  ...
  return (
    <main className="mx-auto max-w-6xl px-6 py-16">
      ...
      <div className="mt-10 flex items-stretch gap-0 overflow-x-auto pb-2">
        {PIPELINE.map((step, i) => (
          <div key={step.label} className="flex flex-shrink-0 items-stretch">
            <div className="flex w-52 flex-col justify-center border border-border bg-surface p-5">
            ...
            {i !== PIPELINE.length - 1 && (
              <div className="flex w-10 flex-shrink-0 items-center justify-center text-text-dim">
        ...
        {Object.entries(toolCounts).map(([tool, data], i, arr) => (
          ...
          >
            ...
            <div className="mt-3 h-1.5 w-full overflow-hidden rounded-full bg-surface-raised">
              <div
                className={`h-full rounded-full ${data.count === 0 ? "bg-fraud" : "bg-review"}`}
                style={{ width: `${(data.count / 12) * 100}%` }}
              />
            </div>
          </div>
        ))}
